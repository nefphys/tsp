        ...
        if tarStruct.isover == 0
            ...
            if tarStruct.in == 0
                ...
                elseif setSize <= 10000
                    ...
                    Centers = int8(setSize/50);
                    K = 10;
                    Clust_Ans = SnnDpc(tempCity,answer,K,'AutoPick',Centers,'Distance',tempCityDist,'Ui',false);
                    %返回一个结构体，包含
                        %NC 簇数
                        %K 参数K
                        ...
                        %center 中心点的id
                    %生成一个Centers个结构体数组
                    tempClustStruct = repmat(std_struct,1,Centers);
                    for h = 1:Centers
                        tempClustStruct(h).set = Clust_Ans.cluster == h
                    end
