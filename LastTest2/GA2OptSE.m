...
function GA2Opt_Struct = GA2OptSE(tspData, startID, endID)
    %判断tspdata是不是大于1的方阵
    %如果是，则传入的是距离矩阵
    tt1 = tic();
    ...
    if CityNum == 2
        ...
        GA2Opt_Struct.route = [startID endID];
        GA2Opt_Struct.bestline = 0;
        GA2Opt_Struct.time = toc(tt1);
    end
    
    if CityNum == 3
        nopd = setdiff(1:3, [startID endID]);
        GA2Opt_Struct.length = distances_matrix(startID, nopd) + ...
            distances_matrix(nopd, endID);
        GA2Opt_Struct.route = [startID nopd  endID];
        GA2Opt_Struct.bestline = 0;
        GA2Opt_Struct.time = toc(tt1);
    end
    
    if CityNum == 4
        nopd = setdiff(1:4, [startID endID]);
        d1 = distances_matrix(startID, nopd(1)) + ...
            distances_matrix(nopd(1), nopd(2)) + ...
            distances_matrix(nopd(2), endID);
        d2 = distances_matrix(startID, nopd(2)) + ...
            distances_matrix(nopd(2), nopd(1)) + ...
            distances_matrix(nopd(1), endID);
        if d1 > d2
            GA2Opt_Struct.length = d2;
            GA2Opt_Struct.route = [startID nopd(2) nopd(1) endID];
            GA2Opt_Struct.bestline = 0;
            GA2Opt_Struct.time = toc(tt1);
        else
            GA2Opt_Struct.length = d1;
            GA2Opt_Struct.route = [startID nopd(1) nopd(2) endID];
            GA2Opt_Struct.bestline = 0;
            GA2Opt_Struct.time = toc(tt1);
        end
    end
    ...
    if CityNum > 4
        %重要参数 对优化结果十分重要
        popSize = 20 + round((CityNum)^0.5 * 1.5 );
        if mod(popSize,2) == 0
        else
            popSize = popSize + 1;
        end
        EAmaxIt = 20 + round((CityNum)^0.5 * 1.5);
        optMaxIt = min(1e5, 1000 + CityNum*15);
        ...
        CityNum = size(City, 1);
        %随机初始化种群
        iniPop = InitPop(popSize, CityNum, startID, endID);
        iniFit = zeros(popSize,1);
        %对解进行2-opt计算
        BKSD = [];
        for i = 1:popSize
            TSP_Struct = CalChrom2OptSE(iniPop(i,:), optMaxIt, CityNum, distances_matrix);
        ...
        for iter = 2:(EAmaxIt+1)
            ...
            for i = 1:popSize
                TSP_Struct = CalChrom2OptSE(nextPop(i,:), optMaxIt, CityNum, distances_matrix);
...
function Chrom = InitPop(popSize, CityNum, startID, endID)
    Chrom = zeros(popSize,CityNum);
    Chrom(:,1) = startID;
    Chrom(:,end) = endID;
    for i = 1:popSize
        Chrom(i,2:(end-1)) = randsample(setdiff(1:CityNum,[startID endID]), CityNum-2);
    end
end
...
function TSP_Struct = CalChrom2OptSE(Chrom, maxIt, CityNum, DistM)
    BKS = zeros(1,maxIt+1);
    CT = CityNum - 2;
    %计算当前染色体的距离
    ndist = 0;
