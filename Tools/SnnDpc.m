...
[dist1Order, dist2, sharedCount, rho, delta, deltaSelect] = CalDist1(dist1, K, N);
...
while unasCount
  ...
  if whichK
    ...
    if K > N
        % 均匀直线数据，直接从中间分割即可
        cluster = repelem(1:KBackup, ceil(N/KBackup));
        cluster = cluster(1:N);
        center = 0;
        for jk = 1:KBackup
            center(jk) = find(cluster == jk, 1);
        end
        break
    end
  end
...
% Calculate dist2
function [dist1Order, dist2, sharedCount, rho, delta, deltaSelect] = CalDist1(dist1, K, N)
    [dist1Sort,dist1Order]=sort(dist1,2);
    %ismembc() needs sorted arrays
    dist1OrderSort=sort(dist1Order(:,1:K),2);
    dist2=zeros(N);
